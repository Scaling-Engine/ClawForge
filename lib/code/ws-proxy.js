...
export function attachCodeProxy(server) {
  ...
  server.on('upgrade', async (req, socket, head) => {
    ...
    // Allow connection if workspace has a container name, even if DB status is stale.
    // The ensureCodeWorkspaceContainer action (called by terminal-view before connecting)
    // handles container recovery. The proxy just needs a container to forward to.
    // Only hard-block if workspace is explicitly destroyed (volume removed, no recovery).
    if (workspace.status === 'destroyed') {
      console.log(`[ws-proxy] rejected: workspace ${workspaceId} is destroyed`);
