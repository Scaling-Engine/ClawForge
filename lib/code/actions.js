...
export async function ensureCodeWorkspaceContainer(id) {
  ...
  try {
    ...
    // Container is in an unrecoverable state (restarting, dead, etc.)
    // Try to force-remove it and mark workspace as destroyed so a new one can be created.
    console.log(`[ensureCodeWorkspaceContainer] container ${workspace.containerName} is ${state}, force-removing`);
    try {
      await container.remove({ force: true });
    } catch {
      // Removal failed — still mark as destroyed
    }
    const { updateWorkspace } = await import('../db/workspaces.js');
    updateWorkspace(id, { status: 'destroyed' });
    clearWorkspaceSessions(id);
    return { status: 'destroyed', message: `Container was ${state} — workspace destroyed. Please start a new session.` };
  } catch (err) {
