...
async function _recreateContainer(workspace) {
  ...
  // Pass through AGENT_LLM_ secrets (API keys for Claude Code CLI)
  // These are in the event handler's environment, passed from GitHub secrets
  for (const [k, v] of Object.entries(process.env)) {
    if (k.startsWith('AGENT_LLM_') && v) {
      env.push(`${k}=${v}`);
    }
  }
...
export async function ensureCodeWorkspaceContainer(id) {
  ...
  try {
    ...
    if (state === 'running') {
      // Check if the container image matches the current workspace image.
      // If the image differs (container was created with an old image), recreate.
      const expectedImage = process.env.WORKSPACE_IMAGE || 'scalingengine/clawforge:workspace-latest';
      const actualImage = info.Config?.Image;
      if (actualImage && actualImage !== expectedImage) {
        console.log(`[ensureCodeWorkspaceContainer] container ${workspace.containerName} has stale image (${actualImage} != ${expectedImage}), replacing`);
        await _recreateContainer(workspace);
        return { status: 'created' };
      }
...
export async function closeInteractiveMode(id, isClean) {
  ...
  try {
    const { destroyWorkspace } = await import('../tools/docker.js');
