...
export async function getBranches(owner, slug) {
  ...
  const { githubApi } = await import('../tools/github.js');
  try {
    const branches = await githubApi(`/repos/${owner}/${slug}/branches?per_page=100`);
    if (!Array.isArray(branches)) return [];
    // Filter out ClawForge internal branches (jobs, workspaces)
    return branches
      .map((b) => b.name)
      .filter((name) => !name.startsWith('job/') && !name.startsWith('clawforge/'));
  } catch {
    return [];
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Cross-agent aggregate Server Actions (Phase 59)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Get agent cards for the agent picker page.
 * Returns enriched instance data with workspace counts and last job time.
 */
export async function getAgentPickerData() {
  await requireAuth();
  try {
    const instances = await getInstancesOverview();
    const { getDb } = await import('../db/index.js');
    const db = getDb();
    ...
    for (const inst of instances) {
      ...
      try {
        const wsRow = db.prepare(
          `SELECT COUNT(*) as cnt FROM code_workspaces WHERE status NOT IN ('stopped', 'deleted')`
        ).get();
        activeWorkspaces = wsRow?.cnt ?? 0;
      } catch {}

      try {
        const jobRow = db.prepare(
          `SELECT created_at FROM job_origins ORDER BY created_at DESC LIMIT 1`
        ).get();
        if (jobRow?.created_at) lastJobAt = jobRow.created_at;
      } catch {}

      // PR count from GitHub — use cached count to avoid slow API calls on picker load
      try {
        openPrs = await getPendingPRCount();
      } catch {}
    ...
    return { error: err.message || 'Failed to load agents' };
  }
}

/**
 * Get pull requests from all agents assigned to the current user.
 * Returns per-agent objects with stale/error indicators for offline agents.
 *
 * @param {string} [state='open'] - PR state: 'open', 'closed', or 'all'
 * @returns {Promise<Array<{ agentSlug: string, stale: boolean, error: string|null, prs: Array }>>}
 */
...
/**
 * Get workspaces from all agents assigned to the current user.
 * Returns per-agent objects with stale/error indicators for offline agents.
 *
 * @returns {Promise<Array<{ agentSlug: string, stale: boolean, error: string|null, workspaces: Array }>>}
 */
...
/**
 * Get cluster runs from all agents assigned to the current user.
 * Returns per-agent objects with stale/error indicators for offline agents.
 *
 * @returns {Promise<Array<{ agentSlug: string, stale: boolean, error: string|null, runs: Array }>>}
 */
...
/**
 * Dispatch a job to a specific agent instance.
 * Verifies the caller is assigned to the agent (or is hub admin).
 * Local instance uses createJob directly; remote instances use Bearer fetch POST.
 *
 * @param {string} agentSlug - Instance name to dispatch to
 * @param {string} jobDescription - Job description text
 * @returns {Promise<{ job_id: string, branch: string, agentSlug: string } | { error: string }>}
 */
