...
/**
 * Fetch ttyd token with retries to handle the race window where the workspace
 * container is marked 'running' but ttyd hasn't started listening yet.
 ...
 */
async function fetchTtydTokenWithRetry(ip, port, maxAttempts = 5, delayMs = 2000) {
  let lastErr;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await fetchTtydToken(ip, port);
    } catch (err) {
      lastErr = err;
      if (attempt < maxAttempts) {
        console.log(`proxyToTtyd: ttyd not ready yet (attempt ${attempt}/${maxAttempts}), retrying in ${delayMs}ms...`);
...
export async function proxyToTtyd(clientWs, ticketData) {
  ...
  //    Retry up to 5 times with 2s delay to handle the race window where the workspace
  //    container is marked 'running' but ttyd hasn't finished starting yet.
  let ttydToken;
  try {
    ttydToken = await fetchTtydTokenWithRetry(ip, port);
  } catch (err) {
    console.error(`proxyToTtyd: failed to fetch ttyd token after retries: ${err.message}`);
