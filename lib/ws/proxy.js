...
export async function proxyToTtyd(clientWs, ticketData) {
  ...
  try {
    ...
    // Use DOCKER_NETWORK env var (includes compose project prefix, e.g. clawforge_noah-net)
    const networkName = process.env.DOCKER_NETWORK || `${ws.instanceName}-net`;
    let network = networks[networkName];
    // Fallback: if exact name not found, search for a network containing the instance name
    if (!network) {
  ...
  //    Without it, ttyd opens the connection but never sends data.
  ...
  upstream.on('open', () => {
    console.log(`proxyToTtyd: upstream OPEN to ${upstreamUrl}`);
  ...
  clientWs.on('message', (data, isBinary) => {
    clientMsgCount++;
    if (clientMsgCount <= 3) console.log(`proxyToTtyd: client→upstream msg #${clientMsgCount} (${data.length} bytes, binary=${isBinary})`);
  ...
  upstream.on('message', (data, isBinary) => {
    upstreamMsgCount++;
    if (upstreamMsgCount <= 3) console.log(`proxyToTtyd: upstream→client msg #${upstreamMsgCount} (${data.length} bytes, binary=${isBinary})`);
  ...
  upstream.on('close', (code, reason) => {
    if (clientWs.readyState === WebSocket.OPEN || clientWs.readyState === WebSocket.CONNECTING) {
      // Sanitize close code — ttyd may send codes outside the valid WebSocket range (1000-4999)
      const safeCode = (code >= 1000 && code <= 4999) ? code : 1000;
      clientWs.close(safeCode, reason || 'Upstream closed');
    }
  });
