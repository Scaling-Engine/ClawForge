...
import { clusterFile, defaultClusterFile } from '../paths.js';
...
export async function loadClusterConfig(filePath) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      // Fall back to the image-baked default in defaults/ (survives Docker volume mounts)
      if (!filePath) {
        try {
          const defaultRaw = await fs.readFile(defaultClusterFile, 'utf8');
          return JSON.parse(defaultRaw);
        } catch {
          // defaults/ also missing — return empty config
        }
      }
      return { clusters: [] };
    }
...
/**
 * Save the cluster configuration to CLUSTER.json.
 *
 * @param {object} config - Config object to write.
 * @param {string} [filePath] - Optional override path (defaults to config/CLUSTER.json).
