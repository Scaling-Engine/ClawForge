...
export async function checkWorkspaceGitStatus(workspaceId) {
  ...
  async function execCollect(cmd) {
    const exec = await container.exec({
      Cmd: cmd,
      AttachStdout: true,
      AttachStderr: true,
  ...
  const porcelain = await execCollect(['git', '-C', '/workspace', 'status', '--porcelain']);
  ...
  try {
    const unpushed = await execCollect(['git', '-C', '/workspace', 'log', '@{u}..HEAD', '--oneline']);
