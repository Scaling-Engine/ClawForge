    ...
    const mounts = [
      {
        Type: 'volume',
        Source: volName,
        Target: '/home/claude-code/workspace',
        ReadOnly: false,
      },
    ];
...
async function _verifyFeatureBranch(container, expectedBranch) {
  try {
    const exec = await container.exec({
      Cmd: ['git', '-C', '/home/claude-code/workspace', 'branch', '--show-current'],
      AttachStdout: true,
      AttachStderr: true,
    });
    const stream = await exec.start({ Detach: false });
...
export async function checkWorkspaceGitStatus(workspaceId) {
  ...
  const porcelain = await execCollect(container, ['git', '-C', '/home/claude-code/workspace', 'status', '--porcelain']);
  ...
  try {
    const unpushed = await execCollect(container, ['git', '-C', '/home/claude-code/workspace', 'log', '@{u}..HEAD', '--oneline']);
...
export async function getRecentWorkspaceCommits(workspaceId, limit = 5) {
  try {
    ...
    const output = await execCollect(container, [
      'git', '-C', '/home/claude-code/workspace', 'log',
 ...
 */
export async function listWorkspaceFiles(workspaceId, basePath = '/home/claude-code/workspace', maxDepth = 3) {
  if (!docker) throw new Error('Docker not initialized. Call initDocker() first.');
