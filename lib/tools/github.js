...
/**
 * Fetch a single file from a GitHub repo via the Contents API (raw mode).
 * Returns the file content as a string, or null if not found / not accessible.
 *
 * Does NOT use githubApi() because it hardcodes Accept: application/vnd.github+json
 * which returns base64-encoded content instead of raw text.
 *
 * @param {string} owner - Repository owner
 * @param {string} repo - Repository name
 * @param {string} filePath - Path within the repo (e.g., '.planning/STATE.md')
 * @param {object} [options]
 * @param {string} [options.ref='main'] - Git ref to fetch from
 * @param {number|null} [options.maxChars=null] - Truncate content to this many characters
...
async function fetchRepoFile(owner, repo, filePath, { ref = 'main', maxChars = null } = {}) {
  try {
    ...
    if (!res.ok) {
      console.error(`fetchRepoFile error: ${res.status} for ${owner}/${repo}/${filePath}`);
      return null;
    }
    ...
    console.error(`fetchRepoFile failed for ${owner}/${repo}/${filePath}:`, err);
    return null;
  }
}
