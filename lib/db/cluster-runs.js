...
/**
 * Get a single agent run by its ID.
 *
 * @param {string} agentRunId - Agent run UUID
 * @returns {object|null} The agent run row or null if not found
 */
export function getAgentRunById(agentRunId) {
  const db = getDb();
  return db.select()
    .from(clusterAgentRuns)
