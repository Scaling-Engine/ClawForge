...
/**
 * Migrate repos from seed files into the DB if DB has no repos.
 * Checks two locations in priority order:
 *   1. defaults/REPOS.json — image-baked seed data, survives config volume mounts
 *   2. config/REPOS.json — legacy location, may be hidden by Docker named volume
...
export function migrateReposFromFile() {
  ...
  // Only migrate if no repos in DB yet (existing row could hold empty array)
  if (existing) {
    // If row exists but is empty, attempt re-seed from defaults
    try {
      const parsed = JSON.parse(existing.value);
      if (Array.isArray(parsed) && parsed.length > 0) return; // Already has repos
    } catch {
      return; // Corrupt row — don't overwrite
    }
    // Row exists but empty — fall through to try seed files
  }

  // Try defaults/REPOS.json first (outside Docker volume mount)
  const seedPaths = [defaultReposFile, path.join(PROJECT_ROOT, 'config', 'REPOS.json')];
