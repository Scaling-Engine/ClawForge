...
import { loadAllowedRepos, resolveTargetRepo, resolveTargetRepoWithFallback, getDispatchMethod, getQualityGates, getMergePolicy } from '../tools/repos.js';
...
/**
 * Build LLM secrets JSON for job containers.
 * DB-stored secrets (admin UI) take priority over env-based secrets (docker-compose .env).
 * @returns {string} JSON string of merged secrets
 */
function buildLlmSecrets() {
  ...
  try {
    ...
    if (dbOAuth) envSecrets.CLAUDE_CODE_OAUTH_TOKEN = dbOAuth;
    if (dbApiKey) envSecrets.ANTHROPIC_API_KEY = dbApiKey;
  } catch {
    // DB not available — use env-only secrets
  }
  return JSON.stringify(envSecrets);
}
...
const createJobTool = tool(
  async ({ job_description, target_repo }, config) => {
    ...
    // Resolve target repo if specified (allowlist first, then GitHub API fallback)
    let resolvedTarget = null;
    if (target_repo) {
      const result = await resolveTargetRepoWithFallback(target_repo);
      if (result.error) {
        return JSON.stringify({ success: false, error: result.error });
      }
      resolvedTarget = result.repo;
    }
  ...
  },
  {
    ...
    schema: z.object({
      ...
      target_repo: z.string().optional().describe(
        'Optional: target repository name or alias (e.g., "neurostory", "ns", "strategyes-lab"). ' +
        'Can be a known alias OR any "owner/slug" GitHub repo the token has access to. ' +
...
async function waitAndNotify(container, jobId, threadId, streamAbort) {
  ...
  try {
    ...
    // Record usage for jobs without an origin thread (no double-recording)
    if (!usageRecorded) {
      try {
        const durationSeconds = Math.round((Date.now() - jobStartTime) / 1000);
        recordUsageEvent({
      ...
      // Consecutive failure alert (MON-02) — no-origin path
      try {
        const { checkAndAlertConsecutiveFailures } = await import('../monitoring/alerts.js');
        const instanceName = process.env.INSTANCE_NAME || 'unknown';
        await checkAndAlertConsecutiveFailures(instanceName);
...
const getProjectStateTool = tool(
  async ({ repo }) => {
    const result = await resolveTargetRepoWithFallback(repo);
    if (result.error) {
      return JSON.stringify({ success: false, error: result.error });
    }
    const resolved = result.repo;
  ...
  },
  {
    ...
    schema: z.object({
      repo: z
        .string()
        .describe(
          'Repository name, alias, or "owner/slug" (e.g., "neurostory", "cf", "Scaling-Engine/n8n"). Works with any GitHub repo the token has access to.'
        ),
    }),
  }
);
...
const startCodingTool = tool(
  async ({ repo }, config) => {
    ...
    // Resolve repo (allowlist first, then GitHub API fallback)
    const repoResult = await resolveTargetRepoWithFallback(repo);
    if (repoResult.error) {
      return JSON.stringify({ success: false, error: repoResult.error });
    }
    const resolved = repoResult.repo;
    ...
    const result = await ensureWorkspaceContainer({
      ...
      llmSecrets: JSON.parse(buildLlmSecrets()),
      threadId,
  ...
  },
  {
    ...
    schema: z.object({
      repo: z.string().describe('Repository name, alias, or "owner/slug" (e.g. "clawforge", "cf", "Scaling-Engine/n8n"). Works with any GitHub repo the token has access to.'),
    }),
  }
);
